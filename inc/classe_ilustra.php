		...
		if($this->album_id > 0)
		{
			...
			if($res = $o_album->selecionar())
			{
				foreach($res as $l_alb)
				{
					...
					{
						$pasta_destino = "galeria_thumbnail";
					}
				}
								...
								switch($this->acao_click)
								{
									...
									case "correspondente":
										if($alt > 0)
										{
											$imagem .= "<div ".$c.">
														<a href=\"".$this->url."\" >
														<img alt=\"".$li["nome"]."\" src=\"".$url_virtual."imagens/".$pasta_destino."/".$li["nome"]."\" width=\"".$larg."px\" height=\"".$alt."px\" /></a></div>";
										}
										else
										{
											$imagem .= "<div ".$c.">
														<a href=\"".$this->url."\" >
														<img alt=\"".$li["nome"]."\" src=\"".$url_virtual."utilitarios/thumbnail.php?largura=".$larg."&altura=".$alt."&img=../imagens/".$pasta_destino."/".$li["nome"]."\" /></a></div>";
										}
									break;
									...
									case "6"://Alan
										...
										$imagem .= "<div class=\"listagem_imagens\"><img ".$c." alt=\"".$li["descricao"]."\" title=\"".$li["descricao"]."\" src=\"".$url_virtual."utilitarios/thumbnail.php?largura=".$larg."&altura=".$alt."&img=../imagens/".$pasta_destino."/".$li["nome"]."\" onclick=\"alert(this.src)\"><a title=\"Eliminar Imagem\" href=\"javascript:ajax_pagina('lista_fotos','excluir', '".$li['id']."', '".$li["id_album"]."', '', '', '', '', '', '', '".$this->div_imagem."', 'ajax_gc_adm', 'false');\">
												<img src=\"".$url_virtual."imagens/site/resp-errada.png\" /></a><input class=\"limpo\" name=\"cmp_".$this->album_id."_img\" type=\"checkbox\" id=\"cmp_".$this->album_id."_img".$cont."\" value=\"".$li["id"]." \"  /> Eliminar <hr> 
												<input class=\"limpo\" name=\"imagem_destaque\" type=\"radio\" value=\"".$li["id"]." \"  onclick=\"javascript:ajax_pagina('foto_destaque','', '".$li['id']."', '".$li["id_album"]."', '', '', '', '', '', '', '".$this->div_imagem."', 'ajax_gc_adm', 'false');\" ".$check."/>Destaque</div>
										".$separador;
									break;
									...
									case "15":
										...
										$imagem .= "<div class=\"".$class."\"><img ".$c." alt=\"".$li["descricao"]."\" title=\"".$li["descricao"]."\" src=\"".$url_virtual."utilitarios/thumbnail.php?largura=".$larg."&altura=".$alt."&img=../imagens/".$pasta_destino."/".$li["nome"]."\"><a title=\"Eliminar Imagem\" href=\"javascript:ajax_pagina('lista_fotos','excluir', '".$li['id']."', '".$li["id_album"]."', '', '', '', '', '', '', '".$this->div_imagem."', '../ajax/ajax_site', 'false');\">
