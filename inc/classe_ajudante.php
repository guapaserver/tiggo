...
class Ajudante
{
	...
	//funзгo para codificar numeros enviados por url
	...
	function ftp_02($ftp_server,$ftp_user_name,$ftp_user_pass,$arquivo_destino)
	{
		...
		{ 
			echo $this->aviso("msg_erro","Erro","Erro ao tentar se conectar a ".$ftp_server." pelo usuбrio ".$ftp_user_name.".");
			exit; 
		}
		else
		{
	...
	function trata_texto_01($string)
	{
		...
		$string = str_replace("Д", "a", $string); 
		$string = str_replace("Е", "a", $string); 
		$string = str_replace("Ж", "ae", $string); 
		$string = str_replace("З", "c", $string); 
		$string = str_replace("И", "e", $string); 
		$string = str_replace("Й", "e", $string); 
		$string = str_replace("К", "e", $string); 
		$string = str_replace("Л", "e", $string); 
		$string = str_replace("М", "i", $string); 
		$string = str_replace("Н", "i", $string); 
		$string = str_replace("О", "i", $string); 
		$string = str_replace("П", "i", $string); 
		$string = str_replace("Р", "d", $string); 
		$string = str_replace("С", "n", $string); 
		$string = str_replace("Т", "o", $string); 
		$string = str_replace("У", "o", $string); 
		$string = str_replace("Ф", "o", $string); 
		$string = str_replace("Х", "o", $string); 
		$string = str_replace("Ц", "o", $string); 
		$string = str_replace("Ш", "o", $string); 
		$string = str_replace("Щ", "o", $string); 
		$string = str_replace("Ъ", "u", $string); 
		$string = str_replace("Ы", "u", $string); 
		$string = str_replace("Ь", "u", $string); 
		$string = str_replace("Э", "y", $string); 
		$string = str_replace("Ю", "a", $string); 
		$string = str_replace("Я", "b", $string); 
		$string = str_replace("а", "a", $string); 
		$string = str_replace("б", "a", $string); 
		$string = str_replace("в", "a", $string); 
		$string = str_replace("г", "a", $string); 
		...
		$string = str_replace("ь", "u", $string); 
		$string = str_replace("э", "y", $string); 
		$string = str_replace("ю", "p", $string); 
		$string = str_replace("я", "y", $string);
		return $string;
	}
	
	function trata_texto_03($string)
	{
		$string = trim($string); 
		// Remove os acentos 
	...
	{
		//nъmero 3 abaixo й nome de ambiente
	...
	function template($nome_aquivo)
	{
		...
		{
			$resultado = "arquivo ".$nome_aquivo." nгo existe";
		}
		else
		{
	...
	function email_html($titulo,$mensagem,$de,$para,$template)
	{
		...
		if(!mail($para, $titulo, $msg, $cabecalho ,"-r".$de)){ // Se for Postfix
			$cabecalho .= "Return-Path: " . $para . $de; // Se "nгo for Postfix"
	...
	function email_html_2($titulo,$mensagem,$de,$para,$comcopiaoculta,$template)
	{
		...
		/* Montando o cabeзalho da mensagem */
	...
	function cria_senha($senha)
	{
		...
		$senha = str_replace("з", "c", $senha);
		$senha = str_replace("у", "o", $senha);
		$senha = str_replace("н", "i", $senha);
		$senha = str_replace("г", "a", $senha);
		$senha = str_replace("х", "o", $senha);
		$senha = str_replace("в", "a", $senha);
		$senha = sha1($senha);
	...
	function data()
	{
		$mes = array("Janeiro","Fevereiro","Marзo","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro");
		$numero_mes = (date("m"))-1;
	...
	{ 
		...
		if ((strlen($email) >= 6) && (substr_count($email,"@") == 1) && (substr($email,0,1) != "@") && (substr($email,strlen($email)-1,1) != "@"))
		{
			if ((!strstr($email,"'")) && (!strstr($email,"\"")) && (!strstr($email,"\\")) && (!strstr($email,"\$")) && (!strstr($email," ")))
			{
				...
				{
					//obtenho a terminaзгo do dominio 
					$term_dom = substr(strrchr ($email, '.'),1); 
					//verifico que a terminaзгo do dominio seja correcta 
	...
	//verifica se usuбrio tб logado
	function verifica_logado()
	{
		if($_SESSION["acesso"] != "sim")
		{
			//Redireciona para pбgina de login
