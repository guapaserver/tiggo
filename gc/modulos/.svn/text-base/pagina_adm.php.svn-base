...
switch($_REQUEST['acao'])
{
	...
	case 'inserido':
		...
		$o_pagina->set('id_pagina_tipo',$_REQUEST['_id_pagina_tipo']);
		$o_pagina->set('id_pagina_estilo',$_REQUEST['_id_pagina_estilo']);
...
switch($_REQUEST['layout'])
{
	case 'form': 
		...
		<strong>Página tipo</strong>
		<select name="_id_pagina_tipo" id="_id_pagina_tipo">
			<option>Selecione a página</option>
			<?php
				
				$o_pagina_tipo = new Pagina_tipo;
				if($rs = $o_pagina_tipo->selecionar())
				{
					foreach($rs as $li)
					{
						if ($l["id_pagina_tipo"] == $li['id']) $select = "SELECTED";
						echo "<option value='".$li['id']."' ".$select.">".$li['tipo']."</option>";
					}
				}
		...
		<strong>Estilo do box</strong>
		<?php
			$o_pagina_estilo = new Pagina_estilo;
			if($rs = $o_pagina_estilo->selecionar())
			{
				foreach($rs as $lin)
				{	
					if($l['id_pagina_estilo'] == $lin['id']){$check = "checked";}else{$check="";}
					echo "<input type=\"radio\"  value=\"".$lin['id']."\" name=\"_id_pagina_estilo\" tabindex=\"7\" ".$check ." > <img src=\"../imagens/site/".$lin['imagem']."\" />";
				}
			}
		?>

		<span class="requerido">*</span>
		<?php echo $o_ajudante->ajuda("Escolha o estilo do box.");?>
		<hr>
