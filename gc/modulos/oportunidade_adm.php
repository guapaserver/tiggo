...
echo $o_ajudante->sub_menu_gc("NOVO","msg=5&acao_adm=oportunidade_adm&acao=nova&layout=form","Oportunidade");
...
switch($_REQUEST['acao'])
{
	...
	case 'inserido':
		...
		$o_produto->set('id_produto_tipo', 2);
		...
		if($o_produto->inserir())
		{
			if($rs = $o_produto->selecionar())
			{
				...
				header("Location: ".$_SERVER['PHP_SELF']."?msg=5&acao_adm=oportunidade_adm&layout=lista");
			}
			else
			{
	...
	case 'editado':
		...
		if($rs = $o_produto->editar())
		{
			...
			header("Location: index.php?acao_adm=oportunidade_adm&layout=lista&msg=1");
		}
		else
		{
	...
	case 'excluir':
		...
		if($rs = $o_produto->selecionar())
		{
			foreach($rs as $l)
			{
				...
				$o_auditoria->inserir();
				header("Location: index.php?acao_adm=oportunidade_adm&msg=8&layout=lista");
			}
		}
		else
		{
...
switch($_REQUEST['layout'])
{
	case 'form': 
		...
		<strong>Titulo da oportunidade</strong>
		...
		if($acao != "inserido")
		{
			...
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:confirma('index.php?msg=6&_id=<?=$l["id"]?>&acao=excluir&acao_adm=oportunidade_adm','<?=$l["nome"]?>')"><img src="../imagens/gc/btn_excluir.png" border="0"></a>
			<?php
		}
		?>
		...
		<input type="hidden" name="acao_adm" value="oportunidade_adm">
