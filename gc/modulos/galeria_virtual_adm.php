...
echo $o_ajudante->sub_menu_gc("NOVO","msg=5&acao_adm=galeria_virtual_adm&acao=nova&layout=form","Galeria Virtual");
...
switch($_REQUEST['acao'])
{
	...
	case 'inserido':
		...
		$o_produto->set('palavra_chave', $_REQUEST['_palavra_chave']);
		...
		if($o_produto->inserir())
		{
			if($rs = $o_produto->selecionar())
			{
				...
				header("Location: ".$_SERVER['PHP_SELF']."?msg=5&acao_adm=galeria_virtual_adm&layout=lista");
			}
			else
			{
	...
	case 'editado':
		...
		if($rs = $o_produto->editar())
		{
			...
			header("Location: index.php?acao_adm=galeria_virtual_adm&layout=lista&msg=1");
		}
		else
		{
	...
	case 'excluir':
		...
		if($rs = $o_produto->selecionar())
		{
			foreach($rs as $l)
			{
				...
				$o_auditoria->inserir();
				header("Location: index.php?acao_adm=galeria_virtual_adm&msg=8&layout=lista");
			}
		}
		else
		{
...
switch($_REQUEST['layout'])
{
	case 'form': 
		...
		<strong>Titulo da Galeria</strong>
		...
		<input id="_palavra_chave" type="text" tabindex="1" maxlength="150" size="50" value="<?=$l["nome"]?>" name="_palavra_chave">
		<hr>
		...
		<div id="div_carrega_imagem">
			...
			<strong>Imagens Disponiveis</strong><br />
		...
		if($acao != "inserido")
		{
			...
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:confirma('index.php?msg=6&_id=<?=$l["id"]?>&acao=excluir&acao_adm=galeria_virtual_adm','<?=$l["nome"]?>')"><img src="../imagens/gc/btn_excluir.png" border="0"></a>
			<?php
		}
		?>
		...
		<input type="hidden" name="acao_adm" value="galeria_virtual_adm">
