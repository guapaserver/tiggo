...
echo $o_ajudante->sub_menu_gc("NOVO","msg=5&acao_adm=noticias_adm&acao=nova&layout=form","Notícias");
...
switch($_REQUEST['acao'])
{
	...
	case 'inserido':
		...
		$palavra_chave = $o_ajudante->trata_texto_03($_REQUEST['_palavra_chave']);
		...
		if($o_produto->inserir())
		{
			if($rs = $o_produto->selecionar())
			{
				...
				header("Location: ".$_SERVER['PHP_SELF']."?msg=5&acao_adm=noticias_adm&layout=lista");
			}
			else
			{
	...
	case 'editado':
		...
		if($rs = $o_produto->editar())
		{
			...
			header("Location: index.php?acao_adm=noticias_adm&layout=lista&msg=1");
		}
		else
		{
	...
	case 'excluir':
		...
		if($rs = $o_produto->selecionar())
		{
			foreach($rs as $l)
			{
				...
				$o_auditoria->inserir();
				header("Location: index.php?acao_adm=noticias_adm&msg=8&layout=lista");
			}
		}
		else
		{
...
switch($_REQUEST['layout'])
{
	case 'form': 
		...
		<input name="_palavra_chave" id="_palavra_chave" type="text" value="<?=$l["palavra_chave"]?>" size="50" maxlength="150" tabindex="1">
		<span class="requerido">*</span>
		<?php echo $o_ajudante->ajuda("Inserir palavras chave para a matéria. Ex.: campeonatos, festas, produtos, etc. Máximo 100 caracteres.");?>
		<div id="div_valida"></div>
		<hr>
		...
		<input type="radio"  value="i" <?php if ($l["estilo"] == "i") echo "checked";?> name="_estilo" id="check_i" tabindex="10"> <img src="../imagens/site/img_inteira.png" onclick="javasrcipt: for_radio('i');"/> 
		<span class="requerido">*</span>
		<?php echo $o_ajudante->ajuda("Escolha o tamanho do box da imagem na home.");?>
		<hr>

		<strong>Tipo da imagem destaque</strong>
		<input type="radio"  value="n" <?php if ($l["estilo_retrato"] == "n") echo "checked";?> name="_estilo_retrato" id="check_n" tabindex="11" > <img src="../imagens/site/img_normal.png" onclick="javasrcipt: for_radio('n');" />
		<input type="radio"  value="r" <?php if ($l["estilo_retrato"] == "r") echo "checked";?> name="_estilo_retrato" id="check_r" tabindex="12"> <img src="../imagens/site/img_retrato.png" onclick="javasrcipt: for_radio('r');"/> 
		<hr>
		...
		<input type="radio"  value="a" <?php if ($l["estado"] == "a") echo "checked";?> name="_estado" tabindex="13"> on-line 
		<input type="radio"  value="i" <?php if ($l["estado"] == "i") echo "checked";?> name="_estado" tabindex="14"> off-line 
		<span class="requerido">*</span>
		...
		if($acao != "inserido")
		{
			...
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:confirma('index.php?msg=6&_id=<?=$l["id"]?>&acao=excluir&acao_adm=noticias_adm','<?=$l["nome"]?>')"><img src="../imagens/gc/btn_excluir.png" border="0"></a>
			<?php
		}
		?>
		...
		<input type="hidden" name="acao_adm" value="noticias_adm">
