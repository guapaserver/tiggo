...
switch($_REQUEST["acao"])
{
	...
	case 'inserido':
		...
		else
		{
			...
			$o_usuario->set('descricao',$_REQUEST['_descricao']);
			$o_usuario->set('usuario_foto',$_REQUEST['usuario_foto']);
			$o_usuario->set('id_usuario_tipo',$_REQUEST['_usuario_tipo_id']);
			$o_usuario->set('id_usuario_02',$_SESSION['usuario_numero']);
			
			$chamada_nome = $o_ajudante->trata_texto_01_url_amigavel($_REQUEST['_login']);
			$chamada_nome = strtolower($chamada_nome);
			$o_usuario->set('chamada_usuario', str_replace(" ", "_", $chamada_nome));
...
switch($_REQUEST['layout'])
{
	case 'form': 
		...
		<form name="form_usuario" class="formularios" id="form_usuario" action="<?=$_SERVER['PHP_SELF']?>" method="post" onSubmit="return CheckRequiredFields();"> 
			...
			<textarea name="_descricao" tabindex="2" cols="27" rows="10"><?= $l['descricao']?></textarea>
			<hr>
			...
			<strong>Foto</strong>
			<input name="usuario_foto" id="usuario_foto" type="text" size="30" readonly="readonly" value="<?=$l["usuario_foto"]?>" />
			...
			<?php echo $o_ajudante->btn_img("Clique para enviar uma nova imagem.","javascript:abre_janela_02('../utilitarios/img_util.php?formulario=form_usuario&campo_nome=usuario_foto&pasta_destino=galeria&img_util=img_procura','img_util','scrollbars=yes,resizable=yes,width=450,height=300');","btn_envia.png","0");?>
			...
			<br>
			<img src="<?=$url_virtual?>utilitarios/thumbnail.php?largura=100&amp;img=../imagens/galeria/<?=$l['usuario_foto'] ?>">
			<hr>

			<strong>Login: </strong>
			<input name="_login" tabindex="16" <?php if ($acao == "editado") echo " readonly=\"readonly\"";?> type="text" id="_login" value="<?=$l["login"]?>" size="30" maxlength="15" onblur="javascript:valida_nome(this.value, '<?=$l['login']?>', 'usuario');"> 
			<span class="requerido">*</span> 
